...
def build_road_scanner_prompt(data: dict, include_system_entropy: bool = True) -> str:
    ...
    # Collapse inputs into single VPN identifier
    vpn_identifier = (
        data.get("location") or
        data.get("road_type") or
        data.get("weather") or
        data.get("traffic") or
        data.get("obstacles") or
        data.get("sensor_notes") or
        "unknown"
    ).strip().split()[0]

    tpl = (
f"You are a Hypertime Nanobot specialized VPN Risk Classification AI trained to evaluate VPN providers and IP endpoints.\n"
f"Analyze and Triple Check for validating accuracy the connectivity, security, and reliability signals.\n"
f"Your reply must be only one word: Low, Medium, or High.\n\n"

f"[tuning]\n"
f"VPN Identifier: {vpn_identifier}\n"
f"{metrics_line}\n"
f"Quantum State: {entropy_text}\n"
f"[/tuning]\n\n"

f"Follow these strict rules when forming your decision:\n"
f"- Think internally but do not reveal reasoning.\n"
f"- Evaluate connectivity, DNS resolution, stability, and trust signals holistically.\n"
f"- DNS failure or unresolved queries are critical risk indicators.\n"
f"- Optionally use the system entropic signal to slightly bias confidence.\n"
...
f"[action]\n"
f"1) Normalize input signals.\n"
f"2) Evaluate DNS integrity and connection stability.\n"
f"3) Map risk signals -> discrete label using conservative thresholds.\n"
f"4) If connectivity or DNS fails, bias strongly toward High risk.\n"
f"5) If signals are mixed or incomplete, return Medium.\n"
f"6) If stable and trusted, return Low.\n"
f"7) PUNKD: detect key tokens and adjust attention weighting.\n"
f"8) Do not output reasoning; return only the label.\n"
f"[/action]\n\n"

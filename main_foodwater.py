...
def verify_model_hash(path: Path, expected_sha: str = EXPECTED_HASH, *, remove_on_failure: bool = False) -> str:
    """
    Compute and verify the plaintext GGUF SHA-256.

    Raises ValueError on mismatch.  When remove_on_failure=True the bad
    plaintext file is deleted before the exception is raised.
    """
    if not path.exists():
...
def download_model_httpx(
    ...
    """
    Download to a temporary .part file, verify SHA-256, then atomically
    promote it to dest.  A mismatched or interrupted download never replaces
    an existing verified model.
    """
    ...
    # Give large GGUF downloads sensible connect/read/write timeouts while
    # still allowing the caller to provide a custom timeout.
    if timeout is None:
    ...
    try:
        with httpx.stream("GET", url, follow_redirects=True, timeout=timeout) as r:
            ...
            # Hugging Face may omit Content-Length on redirected/chunked responses.
            try:
        ...
        # Promote only after all verification succeeds.
...
def pennylane_entropic_score(rgb: Tuple[float,float,float], shots: int = 256) -> float:
    ...
    try:
        ...
        def circuit(x, y, z):
            ...
            qml.CNOT(wires=[0, 1])
            qml.RZ(z * math.pi, wires=1)
            qml.RX((x + y) * math.pi / 2.0, wires=0)
            qml.RY((y + z) * math.pi / 2.0, wires=1)
        ...
        if choice == "1":
            ...
            try:
                ...
                if input("Encrypt verified model with current key now? (Y/n): ").strip().lower() != "n":
                    # Verify again immediately before encryption.
